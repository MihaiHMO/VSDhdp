...
### Yosys and Logic synthesis
  ...
  - `read_verilog <filename>` - read the verilog design file , for more files add all in the command 
  - `synth [option] <module_name>` - this command is telling what is the module we need to synthesize
...
### Hierarchical vs Flat Synthesis
  ...
  The tools will always want to optimize the circuit: 
  - logical (`NOR+INV = NAND`)   
  = electricl - a stacked PMOS can be replaced with a circuit with stacked NMOS     
  
  `flatten` command is eliminating the `sub_modules` and will generate just a big module with the same elements.  
...
+ Re-timing: Lets assume the circuit below and Clk to Q delay Setup and hold time ~ 0.  
...
First example is a flop with `reset` behavior -> a flop was infered (#DFF_PP0)  
...
Dff_const3:  
An example of circuit containing flops with constant value at the input, that will not be optimized because the circuit will toggle based on `reset/set` and `clock` states. The constant value from the input will not propagate at the output of the flop.  
Visible in the waveforms were `Q` and `Q1` are toggling because of values sampled on `clock edge`.   
![](Imgs/l3-8.png)
...
Use cases with different `set/reset` combination:
![](Imgs/l3-9.png)
![](Imgs/l3-10.png)
